...
目前 active baseline 以 `inference/` 的 C++17 `rao_blackwellized_annealed_smc`、`tumor_tree_pipeline/` workflow、ASCAT `rho_ASCAT=0.99` 與 `hcc1395_tumor_tree_input/v4` canonical table 為準。Multiplicity 由 C++ loader 依 major/minor CN 內部建立，不是外部 table input。

## 現況快照

- **目前程式與規格**：`inference/`、`tumor_tree_pipeline/`、`model.md`、`inference_algo.md`、`data.md` 與 `experiment_workflow.md`。
- **目前 canonical input**：`output/tumor_tree_pipeline/input_20260823_old_tagging_v4/`；schema v4、30,490 個 observed sites，其中 30,006 個 eligible，ASCAT `rho_ASCAT=0.99`。
- **目前 PS receipt**：`output/tumor_tree_pipeline/prerequisites_20260823_old_tagging/formal_ps_audit_manifest.json`，PS audit 通過；PS 只作 provenance、QC 與 grouped holdout，不是 likelihood 欄位。
- **目前 formal 結果**：原本唯一保留的 `20260820T122332Z_afce5dea2c6e_...` 失敗診斷 run 已於 2026-08-22 刪除；目前沒有成功的 tumor-tree posterior 可引用。
- **本次清理**：七個舊／失敗 formal run、兩個重複 preflight 目錄、六個舊 blocker receipt、舊 plain-MH sampler、兩份 2026-08-19 HTML、保留的 failed formal run 與兩個舊命名 simulation 檔已刪除；SMC 重新編譯後的 `inference/build/` 保留作為目前可執行的 generated build；以下仍保留其歷史分類。

## A. 目前 active、不要歸入舊資料

| 路徑 | 身分 | 判定依據 |
|---|---|---|
| [`inference/`](inference/) | active C++17 inference backend | AlgorithmRegistry、annealed SMC、canonical loader 與 contract tests |
...
| [`inference_algo.md`](inference_algo.md) | active inference algorithm specification | 定義 `rao_blackwellized_annealed_smc`、particle state、ESS/resampling、rejuvenation 與 repeat I/O |
...
| [`inference/README.md`](inference/README.md) | active backend 說明 | 明確描述 beta schedule、ESS、resampling 與 particle rejuvenation |
...
| `output/tumor_tree_pipeline/input_20260820/manifest.json` | 舊 generated input receipt，需重建 | schema v2、30,490 observed sites、30,006 eligible、`rho_ASCAT=0.99`；與目前 v4 table contract 不同，明確禁止 `tumor_dna_fraction` 與 `multiplicity_posteriors` |
...
[`README.md`](README.md) 已改為連結分離後的 `model.md` 與 `inference_algo.md`，並將 active backend 標示為 annealed SMC；目前不再是 plain-MH stale blocker。

結論：它仍是 repo 入口文件；推理演算法的權威來源改為 [`inference_algo.md`](inference_algo.md)，README 只作導覽。

### B2. 舊 Python plain-MH sampler（已刪除）

`tumor_tree_pipeline/sampler.py` 已於 2026-08-22 刪除。它原本是 legacy Python reference implementation；目前 workflow 的正式 SMC adapter 是 [`cpp_backend.py`](tumor_tree_pipeline/cpp_backend.py)。
...
| `20260823T122155Z_6c535fbf881b_rho0p99-0p97-0p95_K6-4-8_seed20260820` | `6c535fbf881b` | 舊 C++ inference workflow | **已刪除（2026-08-24）**；原先在 sampler 前因 v2 schema 失敗 | 舊輸入介面失敗 run；不可引用 |
| `20260823T123504Z_6c535fbf881b_rho0p99_K6_seed20260823` | `6c535fbf881b` | 舊 C++ compound MCMC | **已刪除（2026-08-24）**；原先 `_SUCCESS`，但不是目前 SMC output | 舊 MCMC 結果；不可引用 |
| `20260823T124632Z_6c535fbf881b_rho0p99_K4-6-8_seed20260823` | `6c535fbf881b` | 舊 C++ sampler diagnostic | **已刪除（2026-08-24）**；原先 `_FAILED`，Rhat/ESS gate 未通過 | 舊診斷結果；不可引用 |
...
`inference/build/` 原先曾是舊 MCMC binary；已於 2026-08-24 重新以目前 SMC source 編譯，現在的 binary 必須以 `--help` 顯示 `rao_blackwellized_annealed_smc` 為準。它仍是可重建的 generated artifact，不是 Git 追蹤的研究輸出。

以下 cache 生成物已於 2026-08-21 刪除：

- `tumor_tree_pipeline/__pycache__/`
- `tumor_tree_pipeline/tests/__pycache__/`

這些目錄不是研究資料，也不是歷史模型結果；之後編譯或執行測試時可以重新產生。其他 ignored checkpoint、samples、logs、temporary lock 與 compiled artifacts 仍不應進入研究上下文，也不應作為來源檔。
...
- 最新 formal failure `20260820T122332Z_afce5dea2c6e_...`、三個 2026-08-23 舊 inference run、舊 plain-MH sampler、兩份 2026-08-19 HTML 與兩個舊命名 simulation 檔已刪除；目前保留新 v4 input、derived counts、PS receipt 與可重建的 validation artifacts，並將 C++/Python inference active contract 收斂到 annealed SMC。
