...
struct Cli {
    std::string algorithm = "rao_blackwellized_annealed_smc";
...
void print_help() {
    std::cout << "tumor_tree_inference (C++17 Rao-Blackwellized annealed SMC backend)\n\n"
              << "Required:\n"
              ...
              << "posterior_summary.tsv.gz, topology_summary.tsv, diagnostics.json,\n"
              << "representative_tree.json, checkpoint.json.gz, smc_complete.json.\n";
}
...
Cli parse_cli(int argc, char** argv) {
    ...
    for (int index = 1; index < argc; ++index) {
        ...
        else if (flag == "--annealing-stages") cli.config.annealing_stages = parse_unsigned<unsigned>(require_value(index, argc, argv, flag), flag);
        ...
        else if (flag == "--repeats") cli.config.repeats = parse_unsigned<unsigned>(require_value(index, argc, argv, flag), flag);
        else if (flag == "--particles") cli.config.particles = parse_unsigned<unsigned>(require_value(index, argc, argv, flag), flag);
    ...
    if (cli.algorithm != "rao_blackwellized_annealed_smc") {
        usage_error("only rao_blackwellized_annealed_smc is available");
    }
    return cli;
}
...
int main(int argc, char** argv) {
    try {
        ...
        std::cerr << "algorithm=" << cli.algorithm << " sites=" << table.sites.size()
                  << " repeats=" << cli.config.repeats << " seed=" << cli.config.seed << "\n";
        ...
        std::atomic<unsigned> next_repeat{0};
        std::mutex error_mutex;
        std::exception_ptr first_error;
        std::vector<tti::RepeatResult> results(cli.config.repeats);
        const unsigned worker_count = std::min(cli.config.repeats, std::max(1U, cli.config.threads));
        auto worker = [&]() {
            while (true) {
                const unsigned repeat_index = next_repeat.fetch_add(1);
                if (repeat_index >= cli.config.repeats) return;
                try {
                    // With multiple repeats, --threads controls repeat parallelism and
                    // each repeat uses one scorer thread to avoid oversubscription.
                    tti::InferenceConfig repeat_config = cli.config;
                    if (cli.config.repeats > 1) repeat_config.threads = 1;
                    tti::RunOptions options;
                    options.outdir = cli.config.repeats == 1
                        ? cli.outdir
                        : cli.outdir / (std::string("repeat_") + (repeat_index + 1 < 10 ? "0" : "") + std::to_string(repeat_index + 1));
                    options.exclude_ids = excludes;
                    auto algorithm = registry.create(cli.algorithm);
                    results[repeat_index] = algorithm->run(table, repeat_config, options, repeat_index);
                } catch (...) {
        ...
        if (first_error) {
            // A multi-repeat invocation is transactional at the completion
            // marker level: a partial run must never look fully complete.
            for (const auto& result : results) {
                if (!result.outdir.empty()) {
                    std::error_code ignored;
                    std::filesystem::remove(result.outdir / "smc_complete.json", ignored);
                }
            }
