...
public:
    ...
    void update(const unsigned char* data, std::size_t length) {
        total_bytes_ += length;
        while (length > 0) {
            const std::size_t take = std::min(length, block_.size() - block_used_);
            std::copy(data, data + take, block_.begin() + static_cast<std::ptrdiff_t>(block_used_));
            block_used_ += take;
            data += take;
            length -= take;
    ...
    std::string finish() {
        ...
        if (block_used_ > 56U) {
            while (block_used_ < block_.size()) block_[block_used_++] = 0;
            transform(block_.data());
            block_used_ = 0;
        }
        while (block_used_ < 56U) block_[block_used_++] = 0;
        for (int shift = 56; shift >= 0; shift -= 8) {
            block_[block_used_++] = static_cast<unsigned char>((bits >> shift) & 0xffU);
        }
        transform(block_.data());

        std::ostringstream result;
        result << std::hex << std::setfill('0');
        for (std::uint32_t word : state_) result << std::setw(8) << word;
        return result.str();
    }
...
std::string sha256_file(const std::filesystem::path& path) {
    ...
    if (!input) throw std::runtime_error("cannot open input for hashing: " + path.string());
    Sha256 digest;
    ...
    if (!input.eof()) throw std::runtime_error("failed while hashing input: " + path.string());
