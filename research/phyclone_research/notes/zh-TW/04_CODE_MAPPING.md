...
| 後代加總 CCF | 主文 §2.2 第 3 頁 | `tree/tree_node.py:35-69`; `tree/utils.py:39-57` | 子節點概似卷積施加父節點 ≥ 子節點總和的限制 | EXACT |
| 邊際化概似 | 主文 §2.2.1 第 3 頁；補充資料 S1.4–S1.5 | `tree/tree_node.py:43-69`; `tree/utils.py:7-68`; `tree/distributions.py:151-163` | 有限網格的後序動態規劃；根節點的總質量固定為 1 | 如文中所述為 APPROXIMATE（近似相符） |
...
| 半調適式提議分布（semi-adapted proposal） | 補充資料 S1.6.2 第 6 頁 | `smc/kernels/semi_adapted.py:31-130` | 評分既有指派，並抽樣新根節點的子節點子集合 | EXACT；基準測試／預設路徑 |
| 完全調適式提議分布（fully adapted proposal） | 補充資料 S1.6.2 第 6 頁 | `smc/kernels/fully_adapted.py` | 列舉所有 `2^R` 個子集合 | EXACT |
| 自助式提議分布（bootstrap proposal） | 補充資料 S1.6.2 第 6 頁 | `smc/kernels/bootstrap.py` | 類似先驗的隨機提議分布 | 未使用離群值時為 EXACT；0.8.0 搭配離群值時為 POSSIBLE_MISMATCH |
| ESS／重抽樣 | 補充資料 S1.6／圖 S4 | `smc/swarm/swarm.py:21-61`; `smc/samplers/standard.py:22-44` | 穩定的正規化權重；在相對 ESS 門檻進行多項分布重抽樣 | ADDITIONAL_IMPLEMENTATION_DETAIL |
...
| 重新指派移動 | 主文 §2.3.2 第 4 頁 | `mcmc/gibbs_mh.py:7-66` | 將非單例叢集中的資料點移至既有節點或離群值狀態；節點數固定 | 行為為 EXACT；正確性有 TODO |
...
| Alpha 重抽樣 | 補充資料 S2.2 第 9 頁 | `mcmc/concentration.py:11-60`; `run.py:292-313,382-392` | 可選的 Gamma 先驗輔助更新 | ADDITIONAL_IMPLEMENTATION_DETAIL |
| 快取／TreeHolder | 補充資料 S2.1–S2.2 第 9–10 頁 | `utils/cache.py`, `hashing_utils.py`, `tree_shell_node_adder.py`, kernels | 有界的卷積／樹／提議分布快取 | EXACT／額外細節 |
...
| 提議分布（proposal） | semi-adapted | semi-adapted | semi-adapted |
...
[明示 STATED] 補充資料 S2.3 說明了三種 PG 取樣核心（kernel）的精確小型後驗測試，以及針對至少十棵樹，以 1,000,000 次 Dirichlet 重要性抽樣進行的邊際化比較（第 10–11 頁）。[實作 IMPLEMENTED] 相對應的測試模組確實存在。在此工作區中，`test_root_term` 的八項測試全數通過。其他選定模組無法在現有 Python 3.10 環境中完成收集，因為缺少必要套件（`numba`、`networkx`、`xxhash`）；儲存庫宣告需要 Python ≥3.12。任何匯入失敗都不會被視為演算法失敗。

## 目前使用時的警告

1. 在 0.8.0 搭配啟用的離群值使用 `bootstrap` 時，應先修正／測試抽樣機率 0.1/0.35/0.55 與所記錄機率 0.1/0.45/0.45 的不一致，再行使用。論文基準測試採用 semi-adapted，因此不受牽連。
2. 除非其選取機率（selection-probability）TODO 已解決，否則應維持關閉子樹 PG。
